...
function CourseCard({ title, description, icon: Icon, color = "blue" }) {
  ...
  const colorClasses = {
    ...
    emerald: "bg-gradient-to-br from-emerald-50 to-emerald-100 border-emerald-200 text-emerald-600",
  ...
  return (
    ...
    >
      <div className="p-6 md:p-8 relative overflow-hidden glass">
        {/* Enhanced background animation */}
        <div 
          className={`absolute inset-0 ${hoverColorClass[color] || hoverColorClass.blue} opacity-0 group-hover:opacity-10 transition-all duration-500`}
          style={{
        ...
        >
          <Icon className={`h-7 w-7 transition-transform duration-300 ${isHovered ? 'scale-125' : ''}`} />
        </div>
        <h3 className="text-xl font-bold mb-3 relative z-10">{title}</h3>
        <p className="text-gray-600 mb-6 relative z-10">{description}</p>
        <a 
          href="#" 
          className="inline-flex items-center text-blue-600 font-medium hover:text-blue-700 relative z-10 overflow-hidden group-hover:font-semibold transition-all duration-300 group-hover:underline underline-offset-4"
        >
          Learn more 
          <ChevronRight 
            className={`h-4 w-4 ml-1 transition-all duration-300 ${isHovered ? 'translate-x-1 scale-110' : ''}`} 
          />
        </a>
      </div>
    </div>
